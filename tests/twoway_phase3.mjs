...
try{
  ...
  const r=await page.evaluate(async()=>{
    ...
    const phases=await import('./src/flow/phases.js?v=2.0.4');
    const {LEAGUE_K,HOF_TH_K}=await import('./src/data/economy.js?v=2.0.4');
    ...
    const audit=(twSeasons)=>{
      ...
      s.traits.genius=true; s.six=5; s.twOrigin='tap';
