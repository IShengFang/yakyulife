import {S, blankStat, bucketOf} from '../core/state.js?v=2.0.8';
import {R, ri, SEED} from '../core/rng.js?v=2.0.8';
import {OFFICIAL_URL} from '../config.js?v=2.0.8';
import {LV, LG_N, CPBL_TEAMS, NPB_TEAMS, MLB_TEAMS, teamNick} from '../data/teams.js?v=2.0.8';
import {TIER_TH, FAN, RP_LV_SUF} from '../data/economy.js?v=2.0.8';
import {TRAIT_KEYS} from '../data/traits.js?v=2.0.8';
import {$, card, choose, divider, board, actClear} from './dom.js?v=2.0.8';
import {careerTimelineCard, tlNote} from './timeline.js?v=2.0.8';
import {traitNames, traitTagStyle, traitColorRank} from './traits.js?v=2.0.8';
import {roleN, fmtIP, slgOf, baseballERA, baseballWHIP, pitG, pitBB} from '../engine/season.js?v=2.0.8';
import {fmtMoney} from '../engine/contract.js?v=2.0.8';
import {isChampionshipYear, isProChampionshipYear} from '../engine/championship.js?v=2.0.8';
import {capTeam, careerMilestones, honorGroups, honorSections, posLegendPhrase, primaryPos, statTable, tierOf, yearRanges, honorText,
  twoWayView, twHasPit, twHasBat, statTables, yrsOf} from '../engine/career.js?v=2.0.8';
import {shareImageSheet} from './share-image.js?v=2.0.8';
/* ================= 結算圖資料建構 =================
...
/* 引退文案要用哪一側的記憶說話。跟表格同一個判斷：看的是「有沒有真的打過二刀流球季」，
   不是退休當下的 S.pos——多數二刀流在生涯尾聲已經被強制轉回單刀。 */
...
export function lateAnswerEnding(pos){
  /* 這一篇整篇是手肘的故事——二刀流的手肘同樣站過投手丘，走投手線。 */
...
你的手肘發炎兩次。你的球速一直卡在某個數字上不去，因為投球隔天你還要打擊，恢復永遠做不完整。
...
export function retireScene(tiers){
  ...
  if(lg==='CPBL'){
    ...
    else if(i===1)txt=`球團為你舉辦了引退儀式。主場滿場，大螢幕播放生涯回顧影片，從高中木棒聯賽夢碎到${twoWayView()?'職棒初登板與職棒初安打':S.pos==='P'?'職棒初登板':'職棒初安打'}，一幕一幕。老隊友從各地回來替你獻花，總教練在致詞時哽咽到說不下去。最後你脫下球帽向四個方向的看板深深鞠躬，應援團的鼓聲直到你走進休息室都沒有停。`;
...
export function endGame(reason){
  ...
  if(reachedTop){
    ...
    /* 努力仔:初始潛力總和偏低(投手≤237/野手≤469/二刀流≤508)。
       三個數字都取各自分布的平均再往下一點點,見 docs/twoway-design.md §4。 */
  ...
  card('','生涯檔案',`隱藏素質：${tr.join(' ')||'（無）'}<br>家庭：${cur}${exStr}｜子女共 ${totKids} 人${lv.affairs?`｜外遇 ${lv.affairs}(${lv.caught})`:''}<br>國際賽出賽：${S.intlCount} 次｜生涯大傷：${S.bigInj} 次${(S.pos==='P'||twoWayView())?`｜手肘危機：${S.tjCrises||0} 次｜Tommy John 手術：${S.tjCount} 次`:''}<br>生涯總薪資：<b class="hl" style="font-size:18px">${fmtMoney(Math.round(S.salary))}</b> 台幣`);
  /* 球迷留言 */
