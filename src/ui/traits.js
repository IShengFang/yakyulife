...
import {$, teamChip} from './dom.js?v=2.0.5';
import {TRAIT_KEYS, TRAIT_N, TRAIT_FX, legendTraitNames, rainbowTraitNames, pitcherTCNames, hitterTCNames, nitenichiNames} from '../data/traits.js?v=2.0.5';
import {TEAM_COLOR, teamNick} from '../data/teams.js?v=2.0.5';
...
export function traitTagStyle(k){
  ...
  if(k==='legend'||k==='taiwan'||k==='intlace'||k==='pitcherTC'||k==='hitterTC'||k==='nitenichi')return 'background:#3a2c05;border-color:#ffc95c;color:#ffe08a'; /* 歷史級/挺台灣/國際賽/三冠王/二天一流:金 */
...
export function traitColorRank(k){ /* 依 traitTagStyle 的顏色分類排序用；同色的特性排在一起，結算列表與結算圖共用 */
  if(k==='legend'||k==='taiwan'||k==='intlace'||k==='pitcherTC'||k==='hitterTC'||k==='nitenichi')return 0; /* 金 */
